...
func LogonScreen(MyWin fyne.Window) fyne.CanvasObject {
	...
	TPbutton := widget.NewButtonWithIcon(config.GetLangs("ls-trypass"), theme.LoginIcon(), func() {
		...
		if !iserrors {
			...
			aliasShadow = config.FyneApp.Preferences().StringWithFallback("NatsAlias", config.Encrypt("MyAlias", config.MySecret))
			alias.SetText(config.Decrypt(aliasShadow, config.MySecret))

			serverShadow = config.FyneApp.Preferences().StringWithFallback("NatsServer", config.Encrypt("nats://nats.newhorizons3000.org:4222", config.MySecret))
			server.SetText(config.Decrypt(serverShadow, config.MySecret))

			queueShadow = config.FyneApp.Preferences().StringWithFallback("NatsQueue", config.Encrypt("MESSAGES", config.MySecret))
			queue.SetText(config.Decrypt(queueShadow, config.MySecret))

			queuepasswordShadow = config.FyneApp.Preferences().StringWithFallback("NatsQueuePassword", config.Encrypt("987654321098765432109876", config.MySecret))
			queuepassword.SetText(config.Decrypt(queuepasswordShadow, config.MySecret))
			queuepasswordShadow = queuepassword.Text

			config.NatsMsgMaxAge = config.Decrypt(config.FyneApp.Preferences().StringWithFallback("NatsMsgMaxAge", config.Encrypt("12h", config.MySecret)), config.MySecret)
	...
	SSbutton := widget.NewButtonWithIcon(config.GetLangs("ls-title"), theme.LoginIcon(), func() {
		...
		if aliasShadow != alias.Text {
			...
			if !haserrors {
				...
				config.FyneApp.Preferences().StringWithFallback("NatsAlias", config.Encrypt(alias.Text, config.MySecret))
			} else {
		...
		if serverShadow != server.Text {
			...
			if !haserrors {
				config.FyneApp.Preferences().StringWithFallback("NatsServer", config.Encrypt(server.Text, config.MySecret))
			} else {
		...
		if queueShadow != queue.Text {
			...
			if !haserrors {
				config.FyneApp.Preferences().StringWithFallback("NatsQueue", config.Encrypt(queue.Text, config.MySecret))
			} else {
		...
		if caShadow != ca.Text {
			...
			if !haserrors {
				config.FyneApp.Preferences().StringWithFallback("NatsCaroot", config.Encrypt(ca.Text, config.MySecret))
			} else {
				errors.SetText(config.GetLangs("ls-err1"))
			}
		}
		if ccShadow != cc.Text {
			...
			if !haserrors {
				config.FyneApp.Preferences().StringWithFallback("NatsCaclient", config.Encrypt(ca.Text, config.MySecret))
			} else {
		...
		if ckShadow != ck.Text {
			...
			if !haserrors {
				config.FyneApp.Preferences().StringWithFallback("NatsCakey", config.Encrypt(ca.Text, config.MySecret))
			} else {
				errors.SetText(config.GetLangs("ls-err3"))
